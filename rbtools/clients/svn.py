...
class SVNClient(SCMClient):
    ...
    DIFF_INDEX_FILE_LINE_RE = re.compile(r'^Index:\s+.*')
    ...
    def do_diff(self, cmd, repository_info=None):
        ...
        diff = self.handle_dir_renames(diff)
        diff = self.handle_renames(diff)
        diff = self.convert_to_absolute_paths(diff, repository_info)

        return ''.join(diff)

    def handle_dir_renames(self, diff_content):
        """
        The output of svn diff confuses ReviewBoard because renamed dir
        paths (which do not exist until after this commit) are probed in
        the repository and thus *do not exist*.

        This method finds instances of this issue, and mangles the new paths
        to use their old path names. Yes, this displays misleading information
        in the ReviewBoard ticket, but is the a way (only way?) to make this work for
        ReviewBoard+Subversion.

        Function `handle_renames(diff_content)` doesn't work for the case of
        directory renames because the files contained within the renamed
        directory aren't considered "renamed" (and don't contain a 'Copied
        From URL' svn info key).
        """
        ...
        # Iterator for maintaining state
        lines = iter(diff_content)
        for line in lines:
            # If this is a "Index: path/to/file.txt" line
            if self.DIFF_INDEX_FILE_LINE_RE.match(line):

                # Get the path (this is a directory)
                path, _ = os.path.split(line[7:])
                info = self.svn_info(path)
                # Check if *any* ancestor dir was renamed
                while path and info and not info.has_key('Copied From URL'):
                    path, _ = os.path.split(path)
                    info = self.svn_info(path)

                # If any dir is being renamed
                if info and info.has_key('Copied From URL'):
                    path     = info['Path']
                    root     = info['Repository Root']
                    url      = info['URL']
                    copy_url = info['Copied From URL']

                    # We want the base directory right in the middle (e.g., a
                    # trunk or branch name). This is important because we must
                    # know what directory the diff was generated in.
                    base     = url[len(root):len(url)-len(path)]
                    new_path = copy_url[len(root)+len(base):]

                    # Mangle the "Index: <old path exists in repo>/file.txt" line
                    result.append(line.replace(path, new_path))

                    # Change the to/from (e.g., '+++ and ---', respectively)
                    # lines.  Iter state ensures lines will not be reprocessed.
                    delim_line = lines.next()
                    orig_line = lines.next()
                    from_line = lines.next()
                    result.append(delim_line)
                    result.append(orig_line.replace(path, new_path))
                    result.append(from_line.replace(path, new_path))

            # Any line not "Index: path/to/file.txt" line
            else:
                result.append(line)
