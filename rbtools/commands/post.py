...
class Post(Command):
    ...
    def post_request(self, repository_info, repository, server_url, api_root,
                     ...
                     diff_history=None, parent_diff_content=None,
                     commit_id=None, base_commit_id=None, submit_as=None,
                     retries=3, base_dir=None):
        ...
        should_validate_diff = (not base_commit_id or
        ...
        if diff_content and should_validate_diff:
            # Validate the diffs to ensure that they can be parsed and that
            # all referenced files can be found. For regular diff validation,
            # we don't need a created review request (and we should do it
            # before the review request is created).
            try:
                validator = api_root.get_diff_validation()

                validator.validate_diff(repository,
                                        diff_content,
        ...
        if (not repository_info.supports_changesets or
            ...
            try:
                ...
                if (base_commit_id and
                    ...
                    # the diff.
                    if diff_history:
                        commit_kwargs['base_commit_id'] = base_commit_id
                    else:
                        diff_kwargs['base_commit_id'] = base_commit_id

                if diff_history:
                    diffs = review_request.get_diffs()
                    diffset = diffs.create_empty_diffset()

                    commits = diffset.get_draft_diff_commits()

                    commit_validator = None

                    if should_validate_diff:
                        commit_validator = \
                            api_root.get_diff_commit_validation()

                    for entry in diff_history:
                        # Validate the commits to ensure that the diffs can be
                        # parsed and that  all referenced files can be found.
                        # This will also validate the commit metadata.
                        if should_validate_diff:
                            try:
                                commit_validator.validate_commit(entry,
                                                                 diffset)
                            except APIError as e:
                                msg_prefix = ''
    ...
    def main(self, *args):
        ...
        if self.options.diff_filename:
            ...
            if with_history:
                history = self.tool.get_history(self.revisions)
                diff = None

                if history:
                    for i, history_entry in enumerate(history):
                        diff_info = self.tool.diff(
                            revisions=self.tool.parse_revision_spec(
                                ['%s..%s' % (history_entry['parent_id'],
                                             history_entry['commit_id'])]),
                            include_files=self.options.include_files or [],
                            exclude_patterns=(self.options.exclude_patterns or
                                              []),
                            extra_args=extra_args)
                        ...
                        if i == 0:
                            base_commit_id = diff_info.get('base_commit_id')
                            history_entry['parent_diff'] = \
                                diff_info.get('parent_diff')
                        else:
                            history_entry['parent_diff'] = None
        ...
        assert diff_content or diff_history
